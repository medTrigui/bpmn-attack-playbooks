    ...
    if (currentPlaybook && modelerRef.current) {
      ...
      if (currentPlaybook.bpmn_xml) {
        modeler.importXML(currentPlaybook.bpmn_xml)
          .then(() => {
            console.log('Playbook imported, waiting for render');
            const eventBus = modeler.get('eventBus');
      ...
      modelerRef.current.importXML(initialDiagram)
        .then(() => {
          ...
          eventBus.once('import.done', () => {
            ...
            if (canvas) {
              try { canvas.zoom('fit-viewport'); } 
              catch (zoomErr) { console.warn('Zoom skipped', zoomErr); }
            }
          });
        })
        .catch(err => {
    ...
    <div className="bpmn-editor-wrapper">
      <div className="editor-toolbar">
        <div className="toolbar-left">
          <button onClick={handleNew} className="btn btn-secondary" title="Create new playbook">New</button>
          <input
            type="text"
          ...
          />
          {currentPlaybook && (
            <span className="loaded-indicator">✓ Loaded: {currentPlaybook.name}</span>
          )}
        </div>
          ...
          <div className="validation-content">
            ...
            {validationResults.errors.length > 0 && (
              <div className="validation-section errors">
                ...
                <ul>
                  {validationResults.errors.map((error, idx) => <li key={idx}>{error}</li>)}
                </ul>
              </div>
            )}
            ...
            {validationResults.warnings.length > 0 && (
              <div className="validation-section warnings">
                ...
                <ul>
                  {validationResults.warnings.map((warning, idx) => <li key={idx}>{warning}</li>)}
                </ul>
              </div>
            )}
          </div>
